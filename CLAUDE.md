...
- Push validation is two-tier, so pushing often to `main` stays fast:
  ...
  - When a conversation looks like it's wrapping up — the requested work is done, no more asks pending, the user's tone reads satisfied — proactively offer the full validation and push (e.g. "On dirait que c'est bon pour cette fonctionnalité — je lance la validation complète et je pousse ?"). Don't wait silently for exact magic words. If the user says no or not yet, accept it gracefully, keep working at the light-gate cadence, and don't re-ask on every message — only when it looks done again.
