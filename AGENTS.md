...
- Push validation is two-tier, so pushing often to `main` stays fast:
  ...
  - Reserve this for a substantial unit of work wrapping up — a real feature or fix touching several files, not a couple of doc lines or a config tweak. Offer it once, naturally, when it's genuinely done (e.g. "On dirait que c'est bon pour cette fonctionnalité — je lance la validation complète et je pousse ?") — not as an immediate reflex the moment anything looks finished. If the user says no or not yet, accept it gracefully, keep working at the light-gate cadence, and don't re-ask until the next substantial wrap-up.
...
- **Work directly on `main` by default.** For a small fix or cosmetic tweak, stay on `main`. But when a request reads as a real feature or substantial development — not "fix this label", but "build this" — proactively propose a worktree *before* starting, and say why: the user routinely runs several Claude terminals on this repo at once, and parallel sessions editing the same files without isolation is a recurring, real source of conflicts. If the user declines, work on `main` as usual.
