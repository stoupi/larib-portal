...
test('a reader trains, calibrates and is certified, then reaches production', async ({ page }) => {
  ...
  await page.getByLabel(/date of exam 1/i).fill('2026-01-15')
  await page.getByRole('dialog').getByRole('button', { name: /create case/i }).click()
  await expect(page.getByText('CAL-MIR-DJ-TEST-002')).toBeVisible({ timeout: 60000 })

  await page.getByRole('button', { name: /assign cases/i }).click()
  await page.getByRole('dialog').getByText('CAL-MIR-DJ-TEST-002').click()
  ...
  await expect(page.getByText(/is not signed/i)).toBeVisible({ timeout: 30000 })
  await page.getByRole('dialog').getByText('CAL-MIR-DJ-TEST-002').click()
