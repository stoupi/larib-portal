    ...
    <div className="app-gradient min-h-full px-4 py-8 md:px-8">
      <div className="mx-auto max-w-[1400px] space-y-6">
        ...
        {membership.canAdjudicate ? (
          <section className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-white p-6">
            <div>
              <h2 className="text-base font-semibold text-text-primary">{t('review.listTitle')}</h2>
              <p className="mt-0.5 text-sm text-text-secondary">{t('review.listSubtitle')}</p>
            </div>
            <Button asChild variant="outline">
              <Link href={`/corelab/studies/${studyId}/reviews`}>{t('review.listTitle')}</Link>
            </Button>
          </section>
        ) : null}
