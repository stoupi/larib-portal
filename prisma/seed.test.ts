...
async function main() {
	...
	await prisma.corelabReviewDecision.deleteMany();
	await prisma.corelabReworkRequest.deleteMany();
	...
	const miniCrf = [
		{
			...
			sections: [{
				...
				fields: [
					...
					{ id: 'wall_motion', name: 'Wall motion', type: 'segment_categorical', required: false, segmentCount: 17, options: ['Normal', 'Akinetic'] },
				],
			}],
		},
	];
	...
	const reviewValues = (lvef: number, esv: number, effusion: boolean) => ({
		values: { EXAM: { cine: {
			...
			wall_motion: { value: { '8': lvef > 46 ? 'Normal' : 'Akinetic' }, source: 'MANUAL' },
		} } },
		flags: [],
		documents: [],
	});
